...
def optimize_google_ads(req: GoogleAdsRequest, x_api_key: Optional[str] = Header(None)):
    _auth(x_api_key)

    if req.dry_run is not None:
        settings.dry_run = req.dry_run

    log.info(f"POST /optimize/google-ads | customer={req.customer_id}")

    from agents import google_ads_agent
    result = google_ads_agent.run(req.customer_id, req.date_range)
    return result


@app.post("/optimize/meta-ads")
def optimize_meta_ads(req: MetaAdsRequest, x_api_key: Optional[str] = Header(None)):
    _auth(x_api_key)

    if req.dry_run is not None:
        settings.dry_run = req.dry_run

    log.info(f"POST /optimize/meta-ads | account={req.ad_account_id}")

    from agents import meta_ads_agent
    result = meta_ads_agent.run(req.ad_account_id, req.date_preset)
    return result
